...
export const createCombine = (combineRoot: string) => {
  ...
  fs.mkdirSync(`src/store-redux/features/${combineRoot}`,{ recursive: true });
  fs.writeFileSync(`src/store-redux/features/${combineRoot}/${combineRoot}.slice.ts`, "");
...
export const createCombineAndFeature = (name: string, combineRoot: string) => {
  ...
  const existCombine = fs.existsSync(`src/store-redux/features/${combineRoot}`);
  const existFeature = fs.existsSync(`src/store-redux/features/${combineRoot}/${name}`);
  ...
  fs.mkdirSync(`src/store-redux/features/${combineRoot}/${name}/use-cases`,{ recursive: true });
  fs.mkdirSync(`src/store-redux/features/${combineRoot}/${name}/reducer`,{ recursive: true });
  fs.mkdirSync(`src/store-redux/features/${combineRoot}/${name}`,{ recursive: true });

  fs.writeFileSync(`src/store-redux/features/${combineRoot}/${name}/${name}.slice.ts`, templateSlice(upName, true));
  fs.writeFileSync(`src/store-redux/features/${combineRoot}/${name}/${name}.module.ts`, templateFeatureModule(upName, true));

  fs.writeFileSync(`src/store-redux/features/${combineRoot}/${name}/use-cases/index.ts`, templateIndexUseCase(upName, true));
  fs.writeFileSync(`src/store-redux/features/${combineRoot}/${name}/use-cases/init.usecases.ts`, templateCreateUseCase("Init", upName, true));
  fs.writeFileSync(`src/store-redux/features/${combineRoot}/${name}/use-cases/save.usecases.ts`, templateCreateUseCase("Save", upName, true));

  fs.writeFileSync(`src/store-redux/features/${combineRoot}/${name}/reducer/${name}-extra.reducer.ts`, templateExtraReducer(upName, true));
  fs.writeFileSync(`src/store-redux/features/${combineRoot}/${name}/reducer/${name}.reducer.ts`, templateReducer(upName, true));
...
export const createFeature = (name: string) => {
  const upName = name.charAt(0).toUpperCase() + name.slice(1);
  const existFeature = fs.existsSync(`src/store-redux/features/${name}`);
